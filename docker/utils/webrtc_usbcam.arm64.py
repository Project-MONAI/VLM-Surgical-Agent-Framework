...
"""
Holoscan-based WebRTC Video Streaming Server

GPU-accelerated video capture and streaming via WebRTC using the
NVIDIA Holoscan SDK. Supports both live USB camera capture and
video file replay.

Usage:
    python test_video_from_usb_cam.py --source camera [--camera-device /dev/video0]
    python test_video_from_usb_cam.py --source file [--video-file /path/to/video]
...
class WebRTCServerApp(holoscan.core.Application):
    ...
    def compose(self):
        ...
        if self._cmdline_args.source == "camera":
            ...
            pixel_fmt = self._cmdline_args.pixel_format
            video_input = V4L2VideoCaptureOp(
                self,
                name="v4l2_source",
                allocator=UnboundedAllocator(self, name="v4l2_allocator"),
                device=self._cmdline_args.camera_device,
                pixel_format=pixel_fmt,
                # Set explicit exposure/gain to prevent Holoscan from trying
                # to set auto-exposure/auto-gain controls which some USB
                # cameras don't support (causing VIDIOC_S_CTRL EINVAL).
                exposure_time=100,
                gain=128,
            )
            # Without pass_through, V4L2VideoCaptureOp converts internally
            # to RGBA8888 on GPU.  FormatConverterOp then converts to RGB
            # for WebRTC.
            logging.info(
                f"V4L2 pixel_format={pixel_fmt} -> internal RGBA -> FormatConverter -> rgb888"
            )
...
def main():
    ...
    source_group.add_argument(
        ...
        choices=["file", "camera"],
        default="camera",
        help="Video source type: 'file' for video file replay or 'camera' for live camera capture (default: camera)",
    )
    source_group.add_argument(
        "--video-file",
    ...
    source_group.add_argument(
        "--pixel-format",
        default="auto",
        help="Camera pixel format as V4L2 fourcc (e.g. YUYV, MJPG, NV12, auto). Default: auto",
    )
    ...
    server_group.add_argument(
        ...
        action="store_true",
        default=False,
        help="Serve embedded web client UI (default: False)",
    )
    server_group.add_argument(
        "--no-serve-client",
        dest="serve_client",
    ...
    # Log configuration
    logging.info("=" * 60)
    logging.info("Holoscan WebRTC Video Streaming Server")
    logging.info("=" * 60)
